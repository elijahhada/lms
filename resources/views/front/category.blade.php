                ...
                {{-- <div class="col-lg-6 col-md-6">
                    <div class="business-home-search">
                        <form method="GET" id="searchform" action="#">
                            <div class="search">
                                <input type="text" name="searchTerm" class="searchTerm" placeholder="What do you want to learn?" autocomplete="off">
                                            ...
                                            <ul>
                                                @if($course->type == 1)
                                                    ...
                                                    @if($course->discount_price == !NULL) 
                                                        ...
                                                        <li class="rate-r">{{ activeCurrency()->symbol }}{{ price_format(  currency($course->discount_price, $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}&nbsp; <br><s>{{ activeCurrency()->symbol }}{{ price_format(  currency($course->price, $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}</s> </li>
                                                        
                                                    @else
                                                        @if($course->price == !NULL)    
                                                        <li class="rate-r">{{ activeCurrency()->symbol }}{{ price_format(  currency($course->price, $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}</li>
                                                        @endif
