            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="bredcrumb-dtl">
                        <h1 class="wishlist-home-heading">{{ __('frontstaticword.Checkout') }}</h1>
                    </div>
                </div>
            </div>
        </div>
		...
		<div class="cart-items btm-30">
	        <div class="row">
	        	<div class="col-lg-4 col-sm-4">
	        		<h4 class="cart-heading bg-white">{{ __('frontstaticword.YourItems') }}
            		(@php
                        if(auth::check())
	        			{
			            		...
			            		<div class="col-lg-9 col-8">
			            			<ul>
		                                ...
		                                @if($cart->offer_price == !NULL)
			            					...
			            					<li class="checkout-course-price"><b>{{ activeCurrency()->symbol }}{{ price_format(  currency($cart->offer_price, $from = $currency->code, $to = $currency->code, $format = false)) }}</b>  <s>{{ activeCurrency()->symbol }}{{ price_format(  currency($cart->price, $from = $currency->code, $to = $currency->code, $format = false)) }}</s></li>
			            					
			            				@else
			            					
			            					<li class="checkout-course-price"><b>{{ activeCurrency()->symbol }}{{ price_format( currency($cart->price, $from = $currency->code, $to = $currency->code, $format = false)) }}</b></li>
	            ...
	            <div class="col-lg-8 col-sm-8">
	            	<div class="checkout-pricelist">
		            	<ul>
		            		...
		            		<li><h1 class="checkout-total">{{ __('frontstaticword.Total') }}: {{ activeCurrency()->symbol }}{{ price_format( currency($cart_total, $from = $currency->code, $to = $currency->code, $format = false)) }}</h1></li>

		            		<li><div class="checkout-price"><s>{{ activeCurrency()->symbol }}{{ price_format( currency($price_total, $from = $currency->code, $to = $currency->code, $format = false)) }}</s></div></li>
